...
$(INSTALLED_BOOTIMAGE_TARGET): $(PRODUCT_OUT)/kernel $(uncompressed_ramdisk) $(recovery_uncompressed_ramdisk) $(INSTALLED_RAMDISK_TARGET) $(INITSH) $(BOOTREC_DEVICE) $(PRODUCT_OUT)/utilities/busybox $(PRODUCT_OUT)/utilities/extract_elf_ramdisk $(MKBOOTIMG) $(MINIGZIP) $(INTERNAL_BOOTIMAGE_FILES)
	...
	$(hide) python $(MKELF) -o $@ $(PRODUCT_OUT)/kernel@0x80208000 $(PRODUCT_OUT)/combinedroot.fs@0x81900000,ramdisk vendor/sony/lbhuashan/proprietary/boot/RPM.bin@0x00020000,rpm device/sony/lbhuashan/rootdir/cmdline.txt@cmdline

	$(hide) ln -f $(INSTALLED_BOOTIMAGE_TARGET) $(PRODUCT_OUT)/boot.elf

	$(hide) rm -fr $(PRODUCT_OUT)/system/lib/modules
	$(hide) cp -r vendor/sony/lbhuashan/proprietary/lib/modules $(PRODUCT_OUT)/system/lib
...
$(INSTALLED_RECOVERYIMAGE_TARGET): $(MKBOOTIMG) \
	...
	$(hide) python $(MKELF) -o $@ $(PRODUCT_OUT)/kernel@0x80208000 $(PRODUCT_OUT)/ramdisk-recovery.img@0x81900000,ramdisk vendor/sony/lbhuashan/proprietary/boot/RPM.bin@0x00020000,rpm device/sony/lbhuashan/rootdir/cmdline.txt@cmdline
